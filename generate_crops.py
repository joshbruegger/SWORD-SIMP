...
class ImageCropper:
    ...
    def __init__(
        ...
        for s in self.to_skip:
            print(f"Skipping {s} as requested")
        ...
        if self.crop_file:
            ...
            with open(self.crop_file, "r") as f:
                ...
                for line in f:
                    ...
                    self.to_regenerate[line[0]].append(list(map(int, line[1:])))
    ...
    @staticmethod
    def generate_random_crop_coordinates(
        image_width, image_height, min_size=1080, max_size=1080, factor=32
    ):
    ...
    def save_crops_and_labels(self, crops, crop_bboxes, crop_labels, filename):
        for i, (crop, boxes, labels) in enumerate(zip(crops, crop_bboxes, crop_labels)):
            cv2.imwrite(str(self.output_images_dir / f"{filename.stem}_{i}.jpg"), crop)
    ...
    def process_image(self, filename):
        ...
        self.save_crops_and_labels(crops, crop_bboxes, crop_labels, filename)
    ...
    def process_images(self):
        ...
        files = [
            ...
            if filename.stem not in self.to_skip
        ]
        histories = [
...
if __name__ == "__main__":
    ...
    print(
        f" Minimum number of bounding boxes per crop: {args.min_crops} ".center(padding)
    )
    print(
        f" Minimum visibility for bounding boxes: {args.min_visibility} ".center(
            padding
        )
    )
    print(
        f" Force the removal of the existing 'cropped' directory if it exists: {args.force} ".center(
            padding
        )
    )
    print(
        f" File containing pre-generated crop coordinates: {args.crop_file} ".center(
            padding
        )
    )
    print(
        f" Number of processes to use for multiprocessing: {args.processes} ".center(
            padding
        )
    )
    ...
    to_skip = []
    with open(Path(args.dir) / "test_painting.txt", "r") as f:
